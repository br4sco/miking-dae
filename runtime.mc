...
let defaultOptions = {
  ...
  benchmarkResidual = None (),
  benchmarkJacobian = None (),
  debug = false
}
...
let argConfig = [
  ...
  ([("--benchmark-res", " ", "<value>")],
   ...
   lam p. { p.options with benchmarkResidual = Some (argToIntMin p 0) }),
  ([("--benchmark-jac", " ", "<value>")],
   "Do not simulate but instead evaluate the jacobian function the given number of times. ",
   lam p. { p.options with benchmarkJacobian = Some (argToIntMin p 0) }),
  ([("--debug", "", "")],
   "Debug runtime. ",
  ...
  = lam numjac. lam varids. lam initVals. lam resf. lam jacYf. lam jacYpf. lam outf.
    ...
    case ParseOK r then
      ...
      if neqi (length r.strings) 0 then
        ...
        match (opt.benchmarkResidual, opt.benchmarkJacobian)
          with (nres, njac) & ((Some _, _) | (_, Some _))
        then
          let sum = ref 0. in
          let randState = lam. create n (lam. int2float (randIntU 0 10)) in
          optionMapOr ()
            (lam nres.
              doLoop nres (lam.
                let y = randState () in
